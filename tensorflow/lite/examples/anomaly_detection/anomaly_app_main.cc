...
int Main(int argc, char** argv) {
  std::string config_path     = "cpu_anomaly_config.json";
  std::string memory_config_path;  // optional memory model config
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--check-libs") {
      ...
      config_path = argv[++i];
    } else if (arg == "--memory-config" && i + 1 < argc) {
      memory_config_path = argv[++i];
